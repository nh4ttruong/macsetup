...
All installable via `brew install --cask <name>` — or just run `./setup application` from [macsetup](https://github.com/nh4ttruong/macsetup).
...
- **[XKey](https://github.com/xmannv/xkey)** — Vietnamese input method for macOS. Lightweight, open-source, works. If you don't type Vietnamese you can skip this one.
  ...
  > Preset config: download with `./setup presets`, then import `XKey-Settings.plist` from the XKey settings panel.
...
- **[zsh-completions](https://github.com/zsh-users/zsh-completions)** — Extra completions for the default shell. Installed by `./setup terminal`.
- **[nvm](https://github.com/nvm-sh/nvm)** — Node.js version manager. Installed by `./setup node` with a configurable default version.
- **[gitprofile](https://github.com/nh4ttruong/macsetup/blob/main/bin/gitprofile)** — My own helper. Switches the local-repo git identity between named profiles (`personal`, `work`, etc). Lives at `/usr/local/bin/gitprofile` after `./setup git`.
