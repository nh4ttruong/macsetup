...
- **gitprofile** - edit the standalone [`bin/gitprofile`](bin/gitprofile) script. The setup's `git` group installs this file verbatim to `/usr/local/bin/gitprofile`, so keep it portable (no setup-specific helpers).
...
Add one `"name | one-line description"` entry to the matching table in [`setup`](setup) - the name arrays and the picker's description lookup are derived from these automatically:

| Type             | Table          |
| ---------------- | -------------- |
| CLI tool (brew)  | `CLI_ITEMS`    |
| GUI app (cask)   | `CASK_ITEMS`   |
| Font             | `FONT_ITEMS`   |

For example: `"ripgrep | Fast recursive grep"`. Keep entries alphabetized within a group when possible, and add a longer note to [AWESOME-MAC-APPS.md](AWESOME-MAC-APPS.md) if the tool deserves one.
