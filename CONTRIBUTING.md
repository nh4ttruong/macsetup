...
- **gitswitch** - edit the standalone [`bin/gitswitch`](bin/gitswitch) script. The setup's `git` group installs this file verbatim to `/usr/local/bin/gitswitch`, so keep it portable (no setup-specific helpers).
